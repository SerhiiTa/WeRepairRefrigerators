...
- COM-01 is complete as the Source Attribution Foundation, and migration `0084_source_attribution_foundation_apply_ready.sql` was successfully applied to Production Supabase. `inbound_sources` is now the durable company-scoped source-attribution registry for websites, microsites, booking widgets, lead generators, tracking sources, and manual sources. Durable nullable attribution fields now exist across Communications -> Intake -> Job through `communication_conversations`, `intake_requests`, and `service_requests`, while existing communication/intake/job source columns remain canonical where already present. COM-01 does not implement public forms, booking APIs, SMS, Telnyx, Retell, Settings UI, Communications UI, analytics, or Scheduling. COM-02 has not started.
- COM-02 is complete as Communication Source Account Capabilities, and migration `0085_communication_source_account_capabilities_apply_ready.sql` was successfully applied to Production Supabase. `communication_source_accounts` now supports explicit inbound/outbound Voice and SMS capability flags plus default outbound Voice/SMS selectors, preserves existing inbound phone source accounts as inbound-voice capable, and enforces one active outbound-default source per company/channel. It does not implement calling, SMS, provider settings UI, forwarding, routing, business hours, recording, AI settings, notifications, provider credentials, or provider API keys.
- COM-03 is complete as the Unified Inbound Event Contract. WRA now has a provider-neutral TypeScript event envelope and normalization/validation helpers for phone, SMS, forms, booking widgets, lead generators, email, social, and manual intake to use before gateway routing. The current Production Retell/Telnyx phone workflow remains unchanged, and no Production migration was required.
- COM-04 is complete as the Unified Intake Gateway Foundation. The gateway accepts `UnifiedInboundEvent`, validates the COM-03 contract, resolves trusted tenant/source context through explicit `inbound_sources` and/or `communication_source_accounts`, and does not infer an `inbound_source` from `sourceAccountId` alone. It preserves attribution snapshot context, exposes idempotency lookup hints for provider/conversation/message identifiers, and returns route decisions for communication events, intake-producing events, and trusted booking candidates without creating Jobs or Appointments. The existing Production Retell/Telnyx phone flow remains unchanged, no Production migration is required, and the Communications / Intake UI pass has not started.
- COMM-UI-01 is complete as the Communications workspace UI pass. `/dashboard/communications` is now the unified dispatcher workspace UI: desktop uses the Inbox / Conversation / context layout, mobile uses Inbox -> dedicated Conversation master/detail navigation, and mobile detail tabs cover Conversation, Customer, Intake, Job, and Activity. Contextual return navigation preserves the originating Communication when opening Customer, Intake, or Job. The page now uses the current WRA mobile drawer/shell, the navigation label is `Communications`, and it is positioned after `Intake`. Intake remains available and has not been removed. Outbound SMS/calling and direct Create Job remain future work. COM-05 has not started, and the next backend roadmap checkpoint remains COM-05.
- COM-05 is complete as the Website / Microsite Source Registration foundation. `public.inbound_sources` remains the website/microsite source registry, and server-side website source normalization plus deterministic trusted source resolution are implemented. No database migration was required, and no Production data or HomeFix domains were seeded. COM-06 has not started. Before COM-06 implementation, perform COM-05.1 Existing Website/Form Integration Audit: inspect the actual existing websites/forms and determine their real submission architecture, current destination, available integration method, and appropriate trusted authentication/signature mechanism. Do not design the public form API around assumptions about existing websites.
- COM-05.1 and COM-05.2 are complete as pre-COM-06 architecture checkpoints. COM-05.1 confirmed the real `werepairsubzero.com` path is static Next.js on GitHub Pages -> Cloudflare Worker -> Telegram notification, with Telegram notification-only long term. COM-05.2 created `docs/COM05_2_TRUSTED_SOURCE_CREDENTIAL_ARCHITECTURE.md` and defines a per-source machine-to-machine credential model for Worker/backend -> WRA ingestion, using separate hashed credentials linked to `inbound_sources`. COM-06, COM-06.1, and COM-06.3 are complete and deployed to Production as the Trusted Website Inbound API foundation plus provider-neutral inbound persistence. Migration `0086_trusted_website_inbound_credentials_apply_ready.sql` was successfully applied to Production after the duplicate audit passed with 0 duplicates for all three unique indexes. `werepairsubzero.com` is registered as an inbound source, the first source credential has been created, and `WRA_INBOUND_SOURCE_CREDENTIAL_PEPPER` is configured in Vercel Production. `POST /api/public/inbound/website` authenticates `Authorization: WRA-Source <public_key>:<secret>` before building a UnifiedInboundEvent from the credential-linked inbound source. Accepted website/booking/lead events persist/reuse an Intake record, create/reuse a website-family Communications conversation/message/timeline context where supported, and return existing persisted identifiers on duplicate retry. COM-06.4 is complete as the website inbound message idempotency fix, and migration `0087_website_inbound_message_idempotency_apply_ready.sql` was successfully applied to Production after the pre-check found 0 duplicate groups. Production retry verification passed: the same `providerEventId` reused the same Intake, Conversation, Message, and Timeline Event. The first real end-to-end booking from `werepairsubzero.com` successfully passed Website Booking Form -> Cloudflare Worker -> WRA Production, appeared in WRA Communications and Intake with `werepairsubzero.com` source attribution, and the existing Telegram notification also succeeded as notification-only. WRA is now the booking persistence authority. Do not manually repeat this integration for every website; the next website architecture direction is a reusable WRA Booking Widget / source configuration. The Dashboard Attention/Action Center audit is complete, but implementation has not started: current Attention is Job-only and incomplete, and future Action Center work is deferred.
...
- Task 165.13 adds the mobile Client-card interaction foundation. Apply `supabase/migrations/0058_client_avatar_persistence_apply_ready.sql` before expecting persistent client avatars. The Client card now loads server-side distance from saved company/technician primary city/state to the job service address, uses no device geolocation, opens address-based Google Maps after confirmation, stores linked-client avatars on `customers.avatar_storage_path` or unlinked job avatars on `service_requests.job_client_avatar_storage_path`, and uses a green SVG phone handset.
- Task 165.14 fixes the unfinished Client-card production path. Avatar upload now resolves ownership from stable job/customer fields before touching migration-only columns, uploads file bytes through the server route, returns signed URLs, and shows upload/error state. New apply-ready migration `0059_base_address_distance_foundation_apply_ready.sql` adds explicit Company Base Address and Technician Base Address override fields plus trusted save RPCs. Distance origin now resolves technician override -> company base address -> missing origin, never browser geolocation.
- Task 165.15 fixes the runtime gaps after `0058`/`0059`. Read-only Supabase verification confirmed the private `client-avatars` bucket exists and is configured correctly, but the service-role DB client received PostgreSQL `42501` on `service_requests`, so a new narrow grant migration `0060_client_avatar_distance_service_role_grants_apply_ready.sql` is required for avatar persistence and client-card distance reads/writes. The avatar API now has a clear `ok/error/avatarUrl/storagePath/ownerType` contract, safe server diagnostics, row-verified DB updates, and explicit file-byte uploads. Distance origin resolution now uses active `company_members` as the company source of truth, falls back from technician override to company base address, treats formatted address as sufficient even without coordinates, and does not expose the private origin address to the browser.
- Task 165.18 fixes the Jobs Center New Job workflow and makes Job Workspace mobile Schedule save use a canonical server-side appointment upsert. Mobile Schedule now calls `PUT /api/service-requests/[id]/appointments`; the server checks real active `appointments` rows and decides `created` versus `updated`. Legacy selected-technician snapshots are reconciled from `selected_technician_slug` before first-time booking. The specified job `33a16f94-0176-4378-8c56-1344ddee9dc3` has no active appointment and no assigned technician id; after slug reconciliation it correctly fails first-time booking because QA Booking Refrigeration does not cover ZIP `10075`. New Job now creates intake then converts directly, without the extra browser-side customer match/create blocker, and duplicate-confirmed New Job intakes are no longer forced to `needs_info`. Browser QA created real unscheduled QA job `7ae0b981-d477-425e-868e-37bd790c6786`; it has a linked customer and converted intake, no appointment, and scheduled fields remain null. Apply `0061_job_workspace_mobile_reschedule_service_role_grants_apply_ready.sql` before verifying existing-appointment update persistence because the current DB still returns service-role `42501 permission denied for table appointments`.
- Task 165.19 fixes the false technician availability block without removing real protections. Root cause: first booking and reschedule paths required an explicit `technician_availability_rules.is_available = true` row even though the scheduling foundation already has default Houston business hours for unconfigured technicians. The route update branch now separates explicit unavailable rules, explicit working-hour rules, default business hours, real overlaps, and self-update exclusion. New apply-ready migration `0062_appointment_availability_default_hours_fix_apply_ready.sql` patches `book_service_request_appointment_rpc(...)` with the same fallback while preserving ZIP coverage, eligibility, duplicate active appointment, and overlap checks. Browser QA created appointment `1500de2b-4889-4635-9d29-ffdfc2609ec0` for service request `7ae0b981-d477-425e-868e-37bd790c6786`; DB verification shows one persisted appointment and mirrored scheduled fields. Apply `0062` after `0061`, then repeat mobile PUT reschedule QA through the mobile Schedule sheet.
- Task 165.23 adds the Job Details catalog/attribution/tag foundation. New apply-ready migration `0063_job_details_catalogs_attribution_tags_apply_ready.sql` creates reusable company/system catalogs for job types, contextual problem types, marketing sources, service tags, and service-request tag relations. Canonical Job Name is `service_requests.job_name`; canonical Description remains `service_requests.issue_description`; `service_requests.request_source` stays the technical intake channel and marketing attribution is separate via `marketing_source_id`. `/api/service-requests/[id]/details` is the single server-side save/read path, and the top Edit Job modal plus lower mobile Details rows now use the same canonical snapshot.
- Task 165.24 corrects the mobile Job Details layout after owner QA. The main Details screen now has only `Job Name`, `Description`, and `Tags`; `Ad Source` moved inside the compound `Job Details` bottom sheet and still saves to `marketing_source_id`. Job Type and Ad Source selectors are collapsed by default and expand into scrollable/searchable panels. Description options are now contextual to the current Job Type instead of dumping all problems, and `0064_job_type_problem_catalog_seed_apply_ready.sql` adds missing default problem seeds on top of the existing `0063` catalog architecture.
...
- Mobile Job Workspace Details now hides the avatar camera badge when a real client photo is loaded. The badge remains only when no avatar photo exists.
- The mobile Schedule card was replaced with a compact borderless Schedule row after Client, using the same divider-first style as the Job Summary and Status rows.
- The whole Schedule row opens a mobile bottom sheet titled `Reschedule` with native start date, start time, end date, and end time fields.
- Existing appointment creation remains through the current booking route; existing appointment rescheduling now uses a narrow authenticated PATCH route that updates the active `appointments` row and mirrors scheduled fields to `service_requests`.
- New migration `0061_job_workspace_mobile_reschedule_service_role_grants_apply_ready.sql` grants only the service-role table privileges needed by that server-side reschedule path. It does not grant direct browser appointment writes or weaken RLS.
- The current scheduling model does not support unschedule/schedule-later. The UI documents that limitation instead of showing a fake toggle.
- Browser QA on the existing local 3002 session verified the Job Workspace route at 390px and sampled 375px / 430px for horizontal overflow with the sheet open. Save was not clicked to avoid mutating the QA job.

## Task 165.17 Reschedule Validation Fix

- Reschedule save no longer treats legacy scheduled jobs as new technician bookings just because `service_requests.appointment_id` is missing.
- Root cause: the mobile sheet selected POST whenever `appointment_id` was absent; POST calls `book_service_request_appointment_rpc`, which correctly performs ZIP coverage validation for initial technician assignment.
- The client now uses PATCH when the job already has scheduled appointment fields and an assigned technician, even if the mirror `appointment_id` is stale.
- The PATCH route can resolve the latest active appointment by `service_request_id` and assigned technician when the explicit appointment id is missing.
- Same-technician reschedules preserve conflict/availability/window/access validation but do not hard-block on ZIP coverage.
- Technician changes are not exposed in the mobile sheet; PATCH rejects attempted technician changes instead of bypassing the full assignment validation path.

## Task 165.26 Mobile Technician Assignment Block

- Mobile Job Workspace Details now has a compact `Technician` row after Job Details.
- The row opens an `Assign Technician` bottom sheet with search, current assignment, initials/avatar color, ZIP coverage hints, availability hints, selected state, and Save/Cancel.
- Canonical assignment field is `service_requests.assigned_technician_profile_id`.
- When an active appointment exists, assignment also updates `appointments.technician_profile_id` while preserving the appointment id, date, and window.
- Unscheduled jobs can be assigned without creating an appointment; Schedule remains not scheduled.
- The trusted server route validates dashboard job access, technician management access, active technician state, service ZIP coverage, appointment availability, and active appointment overlaps.
...
- Mobile Job Workspace Details now stops after the compact Attachments row instead of continuing into large duplicated blocks.
- Hidden on mobile only: `Appliance / Problem`, `Technician findings`, `More job controls`, and `Service address details`.
- The underlying data, notes/findings APIs, workflow actions, address editor, photos, and desktop layout were not removed.
- Attachments now sits immediately under `Technician` with compact Camera and Gallery actions.
- Camera uses `accept="image/*"` and `capture="environment"`; Gallery uses `accept="image/*"`.
- Both actions reuse the existing `service-request-photos` bucket and `uploadTechnicianServiceRequestPhoto(...)` / `add_service_request_photo_rpc(...)` upload path, then refresh photos with `loadPhotos()`.

## Task 165.28 Mobile Attachment Upload Compatibility Fix

- Fixed the mobile Safari/LAN attachment upload runtime failure caused by direct `crypto.randomUUID()` usage in `frontend/src/lib/service-request-photos.ts`.
- New `createSafePhotoStorageId(...)` uses native `globalThis.crypto.randomUUID` when available, falls back to `getRandomValues`, then to a sanitized timestamp/random string.
- Existing storage bucket, photo table, signed URL refresh, MIME validation, and attachment ownership were not changed.
- Attachment upload state now catches thrown upload errors, clears selected file/loading state, and shows a normal retryable error instead of leaving `Uploading photo...`.
- Camera/Gallery still use the same mobile inputs and existing service-request photo upload path.

## Task 165.29 Attachments Preview And Mobile Gallery

- Mobile Job Workspace Details now shows the real job photo count in the Attachments row from existing `service_request_photos` records.
- When photos exist, the row shows a compact newest-first thumbnail strip with up to three signed-URL previews and a `+N` overflow tile.
- Tapping the row opens an internal full-screen mobile Attachments gallery. Tapping Camera or Gallery still opens the existing upload inputs only.

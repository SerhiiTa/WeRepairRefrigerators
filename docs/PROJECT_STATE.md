...
- COM-03 is implemented locally as the Unified Inbound Event Contract. WRA now has a provider-neutral TypeScript event envelope and normalization/validation helpers for phone, SMS, forms, booking widgets, lead generators, email, social, and manual intake to use before COM-04 gateway routing. The current Production Retell/Telnyx phone workflow remains unchanged, no Production migration is required, and COM-04 has not started.

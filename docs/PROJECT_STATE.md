...
- COM-05.1 and COM-05.2 are complete as pre-COM-06 architecture checkpoints. COM-05.1 confirmed the real `werepairsubzero.com` path is static Next.js on GitHub Pages -> Cloudflare Worker -> Telegram notification, with Telegram notification-only long term. COM-05.2 created `docs/COM05_2_TRUSTED_SOURCE_CREDENTIAL_ARCHITECTURE.md` and defines a per-source machine-to-machine credential model for Worker/backend -> WRA ingestion, using separate hashed credentials linked to `inbound_sources`. COM-06, COM-06.1, and COM-06.3 are complete as the Trusted Website Inbound API foundation plus provider-neutral inbound persistence. Migration `0086_trusted_website_inbound_credentials_apply_ready.sql` was successfully applied to Production after the duplicate audit passed with 0 duplicates for all three unique indexes. `werepairsubzero.com` is registered as an inbound source, the first source credential has been created, and `WRA_INBOUND_SOURCE_CREDENTIAL_PEPPER` is configured in Vercel Production. `POST /api/public/inbound/website` authenticates `Authorization: WRA-Source <public_key>:<secret>` before building a UnifiedInboundEvent from the credential-linked inbound source. Accepted website/booking/lead events persist/reuse an Intake record, create/reuse a website-family Communications conversation/message/timeline context where supported, and return existing persisted identifiers on duplicate retry. COM-06.4 is complete as the website inbound message idempotency fix: migration `0087_website_inbound_message_idempotency_apply_ready.sql` was successfully applied to Production after the pre-check found 0 duplicate groups, and website retries now reuse the existing communication message by stable event identity. The Cloudflare Worker/site is not connected yet, and Production verification/deploy of the COM-06.4 code fix is still pending.

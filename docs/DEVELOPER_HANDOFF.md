...
- Task 152 is complete as the First Live Phone Workflow foundation. Apply `supabase/migrations/0052_first_live_phone_workflow_foundation_apply_ready.sql` after `0051`; it adds source-account mapping and server-side Telnyx/Retell-style phone ingestion into Communications Hub plus Intake Inbox. No production phone number, outbound SMS, direct provider-to-job creation, auth change, `.env.local` edit, Estimate AI change, or Task 153 work was added.
...
- Task 152.9B creates `0055_communications_dashboard_user_access_repair_apply_ready.sql` for the production operator `info@refrigeratorhoustonrepair.com`. It repairs `profiles.company_id` plus active `company_members` access to company `f0639d2c-6fcf-4ab5-93a2-cde8f3ba9633` so already-ingested Retell conversations pass company-scoped Communications RLS. It does not change UI, weaken RLS, or require another live Retell call. Task 153 has not been started.
...
- Task 152.9B adds the data-access repair migration `0055_communications_dashboard_user_access_repair_apply_ready.sql`. Apply it after `0054` if `info@refrigeratorhoustonrepair.com` still sees zero conversations despite production Retell rows existing. The migration updates that user's `profiles.company_id` and creates/repairs an active `company_members` row for the production conversation company. No additional paid Retell call is needed; refresh `/dashboard/communications` after applying it.

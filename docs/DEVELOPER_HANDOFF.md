...
Read `docs/WRA_PRODUCT_PRINCIPLES.md` before product, workflow, estimate, intake, AI, dashboard, customer portal, technician, or marketplace work. It is the internal design philosophy for WRA.
...
- Task 150 is complete as Workiz Exit Phase 2 intake foundation. Do not treat it as production telephony or SMS automation; it is the unified review/convert layer before real provider ingestion.
- Task 150.1 is complete as Intake Inbox QA hardening. Apply `0048` after `0047` before live QA.
- Task 150.2 is complete as Intake UI cleanup and conversion access stabilization. Apply `0049` after `0048`; it adds Apt/Unit/Suite persistence, soft dismiss, cleaner helper behavior, and prevents inaccessible converted jobs by resolving company/technician access before inserting the service request. Task 151 has not been started.
- Task 150.3 is complete as Intake duplicate/lifecycle hardening. Apply `0050` after `0049`; it makes conversion idempotent, protects converted intakes from edits, adds archive metadata/status, adds Duplicate Candidates filtering, and keeps hard delete intentionally blocked. Task 151 has not been started.
- Task 150.4 is complete as the Technician Authority estimate architecture pass. Estimate AI must remain an estimate writer only and must not diagnose, infer repairs, invent parts, invent prices, or broaden technician scope.
- Task 150.5 is complete as the product principles and estimate review simplification pass. The new `docs/WRA_PRODUCT_PRINCIPLES.md` is mandatory context, the estimate review card now focuses on draft summary, repairs included, parts included, and missing information, and normal production UI should not expose confidence/language/normalization/prompt internals. Task 151 has not been started.
- Task 150.6 is complete as the estimate approval immediate refresh fix. Public approval now shows approving/declining state, updates local customer UI immediately after success, rehydrates from refreshed estimate data, and Job Workspace refreshes job/estimate state on tab focus. Task 151 has not been started.
- Task 151 is complete as the Communications Hub foundation. Apply `supabase/migrations/0051_communications_hub_foundation_apply_ready.sql` before expecting live conversation persistence. `/dashboard/communications` is the minimal Calls & Messages workspace. No production Telnyx, Retell, SMS/calls/email, auth, `.env.local`, Settings, Estimate AI, or Task 152 work was added.
- Task 152 is complete in Production as the First Live Phone Workflow. Real Retell `call_analyzed` payloads now reach production, match source phone `+13466461949`, create Communications Hub conversation/transcript/message/timeline rows, create Intake Inbox rows, and display existing production calls in `/dashboard/communications`. No outbound SMS, direct provider-to-job creation, auth change, `.env.local` edit, Estimate AI change, or Task 153 work was added.
- Task 152.1 is complete as Retell webhook ingestion stabilization. The endpoint now supports Retell `event = call_analyzed` payloads with data under `call.*`, safe structural Vercel diagnostics, source matching for `+13466461949`/digits/formatted variants, and idempotent repeated `call_id` behavior. Task 153 has not been started.
- Task 152.5 adds `supabase/migrations/0053_communications_service_role_grants_apply_ready.sql` after production Retell QA confirmed PostgreSQL error `42501` (`permission denied for table communication_source_accounts`) during service-role source lookup. The final grant set requires `SELECT, INSERT` on `intake_requests` because phone ingestion uses `insert(...).select("id").single()`; `GRANT SELECT ON public.intake_requests TO service_role;` was manually applied in production after the first `0053` apply. Task 153 has not been started.
- Task 152.6 fixes the intake insert schema mismatch after production returned `PGRST204` for missing `duplicate_confirmed`. Do not add a `duplicate_confirmed` column; it is an RPC-only JSON control flag. Phone ingestion strips it before direct `intake_requests` insert. Task 153 has not been started.
...
- Task 162 is complete as documentation-only Job Workspace audit. Read `docs/TASK162_JOB_WORKSPACE_AUDIT.md` before any future Job Workspace redesign. The audit recommends keeping the operational capabilities but restructuring the screen around a compact header, one primary Next Action, merged current job facts, collapsed appointment/address/history details, and clearer estimate/invoice/finance separation.
- Task 163 is complete as Property Intelligence infrastructure. `/api/property-intelligence` is a dashboard-authenticated server route that calls HasData Zillow server-side only, returns the minimal `PropertyIntelligence` contract, and caches normalized-address lookups. No Job Workspace UI was changed.
...
- The post-Task-164 address/property repair is complete and owner visual QA passed externally. The accepted real address is `20406 Ranger Point Ct, Katy, TX, 77450`. Root cause: the Property Intelligence ZIP validator selected street number `20406` as the expected ZIP, rejected a valid HasData/Zillow response with ZIP `77450`, and returned `property: null`. `frontend/src/server/property-intelligence/hasdata-zillow.ts` now uses the last ZIP-like token, supports the current HasData fields (`image`, `zestimate.zestimate`, `area.livingArea`, `homeType`, `geo`, `staticMapUrls`), and uses cache namespace `property-intelligence-hasdata-zillow-v5`.
- Maps and Distance were intentionally not changed. They continue to use the Job service address as the primary address and coordinates only as fallback. Property Intelligence must also continue using the current Job service address and must not depend on `customer_addresses` existing.
- Address workflow migrations `0068_customer_job_address_workflow_repair_apply_ready.sql`, `0069_customer_address_normalized_dedupe_finalize_apply_ready.sql`, and `0070_customer_address_state_uppercase_finalize_apply_ready.sql` were applied manually. They preserve human-readable customer addresses while deduping Court/Ct, Boulevard/Blvd, Street/St, Apartment/Apt, spacing, punctuation, capitalization, and ZIP+4 variants. New states persist uppercase (`TX`). Existing `TE` or lowercase `tx` QA rows are legacy data; do not rewrite/delete them unless a future data-cleanup task explicitly asks.
...
1. Task 149 — Professional Job Workspace Completion: complete. The Job Workspace Overview now gives technicians a Next Action, call/text/email shortcuts, appliance/diagnosis summary, Repair Intelligence entry point, estimate/invoice status, parts workflow controls, technician findings capture, and one-tap lifecycle status actions from the existing job screen.
2. Task 150 — Unified Intake Inbox Foundation: complete. `/dashboard/intake` now lets dashboard users review incoming requests from manual, website, phone/SMS/email, Google/Yelp, and future Retell AI sources, edit extracted fields, set status/source, prepare appointment fields, and convert qualified intake into real jobs through trusted RPCs. Apply `supabase/migrations/0047_unified_intake_inbox_foundation_apply_ready.sql` before live persistence.
   - Task 150.1 hardening: complete. Apply `supabase/migrations/0048_intake_inbox_qa_hardening_apply_ready.sql` after `0047`. It adds split names, address metadata, duplicate candidate review, hardened conversion access checks, and UI helpers/validation.
   - Task 150.2 stabilization: complete. Apply `supabase/migrations/0049_intake_conversion_access_and_ui_cleanup_apply_ready.sql` after `0048`. It fixes the remaining inaccessible converted-job path by resolving an effective company or technician slug before insert, adds unit capture, hides dismissed intakes from the default Active filter, removes duplicate Customer Name entry, and keeps Task 151/provider work unstarted.
   - Task 150.3 lifecycle hardening: complete. Apply `supabase/migrations/0050_intake_lifecycle_duplicate_archive_hardening_apply_ready.sql` after `0049`. It prevents repeated conversion from creating duplicate jobs, returns existing linked jobs for retry/double-click cases, adds `archived`, protects converted intakes, and blocks hard delete in favor of archive.
   - Task 150.4 estimate architecture redesign: complete. Estimate AI is a technician-authorized estimate writer, not Repair Intelligence.
   - Task 150.5 product principles and estimate review simplification: complete. The estimate review should feel like reviewing a prepared estimate, not debugging AI.
...
- Task 149 completed Workiz Exit Phase 1 for the Job Workspace. Audit found that the workspace already had customer/job summary, status updates, address/maps, dispatcher preview, appointment booking, Repair Intelligence estimates, invoices, notes, photos, and timeline, but still lacked an operational flow center. `ServiceRequestDetail.tsx` now surfaces a prominent Next Action, device call/text/email shortcuts, one-tap workflow status actions, appliance and diagnosis context, current estimate/invoice state, technician findings capture through existing diagnostic notes, parts lifecycle status controls, and a current-job history summary. This is workflow completion only: no schema, migrations, new APIs, SMS automation, payment work, dispatcher board/calendar changes, vendor marketplace, inventory, community, or Task 150 work was added.

- Task 150 completed the Unified Intake Inbox foundation. The audit found current intake/job creation was distributed across `/api/service-requests`, customer asset booking, appointment booking, and Job Workspace management, with no single pre-job review queue. Task 150 adds `supabase/migrations/0047_unified_intake_inbox_foundation_apply_ready.sql`, `frontend/src/server/intake/`, authenticated `/api/intake/*` routes, disabled-safe website and Retell/Telnyx webhook foundations, and `/dashboard/intake`. Dispatchers can list, filter, review, edit, status, and convert intake records into real CRM jobs; appointment creation is supported when reviewed date/window/technician data is available. This task did not change auth, login/register, `.env.local`, provider secrets, existing service request flows, existing appointment flows, estimates, customer portal, dashboard behavior, SMS/calls, or production telephony.

- Task 150.1 fixed Intake Inbox QA issues after `0047` was applied. Root cause of the converted-job 403 was conversion creating service requests with `assigned_technician_profile_id` but without the selected public technician slug/business snapshot needed by existing `can_view_service_request()` fallback access. `0048_intake_inbox_qa_hardening_apply_ready.sql` replaces create/update/convert RPCs so split customer names, address metadata, duplicate metadata, and selected technician slug snapshots persist. Conversion now verifies the created job is accessible before marking the intake converted, so failed or partial conversion keeps the intake unconverted. The UI now includes first/last names, Google Places address autocomplete with manual fallback, appliance and brand helper chips, appointment window validation, duplicate review confirmation, and clearer Open Job behavior. Remaining Retell/Telnyx work is provider verification, idempotency, real inbound payload storage, and production automation. Task 151 was not started.

- Task 150.2 cleaned up the remaining Intake Inbox QA friction. `0049_intake_conversion_access_and_ui_cleanup_apply_ready.sql` adds `intake_requests.unit`, persists Apt/Unit/Suite through create/update, and replaces conversion so it establishes dashboard visibility before inserting the service request. Conversion now resolves an effective company from intake/company membership/profile/technician data or falls back to the converter's own technician profile slug; if neither access path exists, it fails before creating the job. The intake UI now edits first/last name only, generates legacy `customer_name` internally, closes address suggestions after selection, shows appliance/brand suggestions only while editing, expands helper vocabulary, uses `Create Intake` / `Save Intake` as the bottom primary action, and supports soft `Dismiss Intake`. Dismissed intakes are hidden from the default Active filter but available via Dismissed/All.

- Task 150.3 hardened Intake lifecycle behavior before real provider traffic. `0050_intake_lifecycle_duplicate_archive_hardening_apply_ready.sql` adds `archived`, `dismissal_reason`, `archived_at`, and `archived_by`, replaces conversion with an idempotent RPC that returns an existing linked job instead of creating duplicates, and blocks conversion for dismissed/archived unlinked intakes. Converted intakes are read-only in the inbox and should be edited only from the linked Job Workspace. The inbox filters now include Active, New, Needs Review, Ready to Convert, Converted, Dismissed, Archived, Duplicate Candidates, and All. Hard delete is intentionally unavailable; archive old converted/dismissed intakes and use existing job statuses (`canceled`/`closed`) for duplicate test jobs. Task 151/provider automation was not started.

- Task 151 created the Communications Hub foundation. Read `docs/COMMUNICATIONS_HUB_FOUNDATION_TASK151.md` before changing communications. `0051_communications_hub_foundation_apply_ready.sql` creates `communication_conversations`, `communication_messages`, `communication_transcripts`, and `communication_timeline_events` with company/profile-scoped RLS and an authenticated create RPC. `frontend/src/lib/communications/` contains customer recognition helpers, business-only timeline filtering, and disabled provider adapters. `/dashboard/communications` shows conversations and business timeline events only; it must not expose provider debug details, AI internals, confidence, language detection, parsing, or ZIP validation in normal technician workflow. Telnyx and Retell remain unconnected.

- Task 152 created the First Live Phone Workflow foundation. Read `docs/FIRST_LIVE_PHONE_WORKFLOW_TASK152.md` before changing phone ingestion. `0052_first_live_phone_workflow_foundation_apply_ready.sql` adds `communication_source_accounts` and call metadata columns so incoming phone events must map to an owned company source before WRA writes records. `frontend/src/server/communications/phone-normalization.ts` and `phone-workflow.ts` normalize Telnyx/Retell-shaped payloads, resolve the source account, match existing customers by phone/email, create/reuse conversations, store transcript/message content, add an `Incoming call` business timeline event, and create Intake Inbox records. `POST /api/communications/phone-webhook` is the new phone endpoint, and the old `/api/intake/webhook-retell-telnyx` path now uses the same handler. Provider payloads intentionally do not create service requests or appointments directly; dispatcher conversion through Intake remains the safe path until source verification, idempotency, appointment validation, and SMS consent are proven. No auth, `.env.local`, production phone number, outbound SMS, Retell production agent, Estimate AI, or Task 153 work was added.

- Task 152.1 fixed Retell ingestion after Vercel showed `POST /api/communications/phone-webhook 202` but no records were created. Root cause: the webhook was reachable, but the first normalizer did not fully map Retell's real `call_analyzed` shape under `call.*`, and source matching needed to compare owned phone variants such as `+13466461949`, `13466461949`, and formatted equivalents. Safe logs now show event, top-level/call/custom-analysis keys, call id, from/to number, transcript presence/length, source match/skip reason, and completion status without logging full transcripts, raw payloads, headers, keys, or secrets. Repeated `call_id` reuses existing conversation/intake and avoids duplicate transcript/message/timeline rows. After deploy, place a real Retell test call and verify rows in `communication_conversations`, `communication_transcripts`, `communication_timeline_events`, and `intake_requests`.

- Task 152.5 created `0053_communications_service_role_grants_apply_ready.sql`. Production Retell QA reached the webhook and produced safe diagnostics, but Supabase returned `code: 42501`, `message: permission denied for table communication_source_accounts`, `operation: select_source_account`. This is a PostgreSQL grant issue for the server-side `service_role` role. `0053` grants only the minimum phone-ingestion privileges: source-account select, conversation select/insert/update, transcript/message/timeline select/insert, intake select/insert, and customer select. It does not disable RLS, weaken anon/authenticated policies, change auth, change architecture, or start Task 153.

- Task 152.6 fixed the next production blocker without another live call. After conversation creation, `intake_requests` insert failed with `PGRST204` because the direct service-role insert included `duplicate_confirmed`, which is only a JSON control flag for dashboard intake RPCs. The physical table stores `duplicate_confirmed_at` and `duplicate_confirmed_by`. `phone-workflow.ts` now removes `duplicate_confirmed` before table insert. No migration is required or desired for this fix.

- Task 152.7 fixed Communications Hub read/display readiness. The rows were real, not mock: production contains Retell `communication_conversations` with `provider_name = retell`, `primary_source_type = phone`, `status = needs_action`, call start/end timestamps, summaries, next actions, and at least one linked `intake_request_id`. `CommunicationsHub.tsx` now selects and displays these fields. `0054_communications_dashboard_visibility_profile_company_apply_ready.sql` updates the RLS helper to preserve owner/creator/company-member access and add a narrow active `profiles.company_id = conversation.company_id` dashboard compatibility path for provider-created rows where `owner_profile_id` and `created_by` are null. Do not disable RLS or make communications public.

- Task 152.9B/152.9C clarified the production dashboard access repair. The successful repair was company-members-only. Do not document or reintroduce any direct `profiles.company_id` update for this fix; that path is intentionally blocked by `prevent_unsafe_profile_updates()`. Refreshing `/dashboard/communications` after the company membership repair shows existing production calls. No additional paid Retell call is needed for Task 152.
...
- Task 150.4 redesigns Estimate AI around the Technician Authority Principle. `/api/estimate-agent/draft` must remain an estimate-writing endpoint, not a diagnosis or Repair Intelligence endpoint. It may rewrite technician notes, improve grammar, organize explicit labor/part/service lines, generate customer-friendly explanations, and surface missing-information placeholders. It must not diagnose, infer failed components, add common-practice repairs, invent parts/labor/prices/quantities, or broaden technician scope. Do not reintroduce repair-scope inference into this route.
- Future Repair Intelligence should be a separate subsystem: Repair Intelligence -> Validated Repair Scope -> Estimate Builder -> Pricing Engine -> Customer Estimate. The Estimate Builder should receive structured `repair_operations`, `replacement_parts`, `labor_operations`, `customer_prices`, and technician notes, then format them without deciding what repairs exist.
- If technician input contains only symptoms, such as an LG linear compressor running with no cooling, Estimate AI should request `[TECHNICIAN CONFIRMATION REQUIRED]` instead of creating compressor/sealed-system/refrigerant/filter-drier lines. If technician explicitly says to replace/change/defrost an item, the estimate must preserve that item and use `[PART PRICE REQUIRED]` or `[LABOR PRICE REQUIRED]` rather than guessing.
...
- Retell recording playback is internal-only in `/dashboard/communications`; it uses the selected conversation id plus the logged-in dashboard session to call the server route, which fetches Retell metadata with server-only `RETELL_API_KEY`.
...
- Main stats are Jobs, Active, and Gross only. Jobs is the selected-month job count. Gross is a safe selected-month paid invoice read, not compensation/accounting logic.
...
- Read `docs/TASK162_JOB_WORKSPACE_AUDIT.md` before changing `frontend/src/components/dashboard/ServiceRequestDetail.tsx`.
- Task 162 was documentation only. It did not modify app code, backend logic, Supabase, Retell, Communications Hub, Jobs Center, authentication, or database schema.
- Keep: compact header, job name, current status, customer/contact, service address/map, appliance/problem, technician findings, photos, estimate workflow, invoice workflow, and timeline/history access.
- Merge: repeated customer, service address, appointment, appliance, issue description, status, finance, and current-job fact blocks.
- Collapse: address editor, appointment matching diagnostics, parts workflow, estimate history, invoice history, timeline/history, and technical scheduling details.
- Remove from the default overview in the future implementation pass: old repeated detail grid, duplicate issue-description card, customer repair history counters without real previous repairs, large disabled unavailable action blocks, and coordinate/navigation metadata.
## Task 163 Property Intelligence Foundation

- Server API: `frontend/src/app/api/property-intelligence/route.ts`.
- Shared type: `frontend/src/lib/property-intelligence.ts`.
- Server provider boundary: `frontend/src/server/property-intelligence/hasdata-zillow.ts`.
- The route accepts `GET ?address=` or `POST { address }`, requires the existing dashboard bearer token, and returns `{ property: PropertyIntelligence | null }`.
- `PropertyIntelligence` intentionally exposes only `photo`, `zestimate`, `livingArea`, `yearBuilt`, `propertyType`, `latitude`, `longitude`, and `mapImage`.
- The HasData key is read from server env `HASDATA_API_KEY` only. Do not add `NEXT_PUBLIC_HASDATA_API_KEY` or call HasData from browser code.
- Results are cached with Next server data cache for 14 days by normalized address. If the provider misses, fails, or returns an unsupported shape, the route returns `property: null` so the future Job Workspace can degrade cleanly.
- This task did not modify the Job Workspace UI. Future UI work should consume this route rather than calling HasData directly.

## Task 164 Job Workspace Property Preview

- UI file: `frontend/src/components/dashboard/ServiceRequestDetail.tsx`.
- Documentation: `docs/TASK164_JOB_WORKSPACE_PROPERTY_PREVIEW.md`.
- The Overview tab now shows a compact Property Preview card after the `Next action` card.
- The card calls `/api/property-intelligence` with the current job service address and the existing dashboard bearer session. It skips the request when the job has no street/full service address.
- The card renders only `mapImage`, `photo`, `zestimate`, `livingArea`, and `yearBuilt`; all other Zillow/listing fields remain hidden.
- Provider/API failures and `property: null` results show a compact placeholder instead of a technician-facing error.
...
- Read `docs/FINANCE_WORKIZ_EXIT_ROADMAP.md` and `docs/TASK166_PRICE_BOOK_FOUNDATION.md` before future estimate, invoice, payment, deposit, or pricing work.
- Task 166 created `supabase/migrations/0066_price_book_foundation_apply_ready.sql`.
- Apply `0066` before using Settings -> Price Book against Supabase.
- New API route: `frontend/src/app/api/settings/price-book/route.ts`.
- New Settings component: `frontend/src/components/dashboard/PriceBookSettings.tsx`.
- Settings page now renders `PriceBookSettings` below Company Base Address.
- New canonical tables are `price_book_appliance_groups`, `price_book_appliance_group_types`, `price_book_items`, `price_book_item_aliases`, `price_book_bundle_items`, and `price_book_item_merge_history`.
- Browser clients should not write directly to these tables. Use `/api/settings/price-book` or future trusted RPC/API paths.
- Existing estimate tables/RPCs and invoice tables/RPCs remain the current finance document engines. Do not fork them.
- Next finance implementation should be Manual Estimate Builder from Price Book, not AI-first estimate generation, invoice, deposit, or payment work.

## Task 167 Price Book UX And Selection Foundation

- Read `docs/TASK167_PRICE_BOOK_UX_AND_SELECTION.md` before continuing Finance work.
- Primary UI file: `frontend/src/components/dashboard/PriceBookSettings.tsx`.
- Settings Price Book now uses appliance/category navigation and simple repair rows. Do not reintroduce visible columns/cards for aliases, keywords, duration, tax, warranty, labor split, parts split, or bundle internals on the first-level list.
- The repair editor is the single place for create/edit, bundle content, duplicate warnings, warranty, matching, and internal pricing.
- API file: `frontend/src/app/api/settings/price-book/route.ts`.
- Important save behavior: system/default rows (`company_id = null`) must not be mutated by company users. PATCH creates a company-owned override and the UI collapses it over the default row.
- Future Finance suggestion endpoint: `frontend/src/app/api/price-book/repair-solutions/route.ts`.
- Future Finance ranking helper: `frontend/src/server/price-book/selection.ts`.
- The suggestion endpoint is rule-based only and returns `aiUsed: false`. It does not create estimates, invoices, payments, deposits, or line items.
- Refrigerator contexts must not return Dryer-only repairs. General items are allowed only as lower-priority fallback suggestions.

## Task 167.9 Finance System Master Plan

- Read `docs/FINANCE_SYSTEM_MASTER_PLAN.md` before any future Price Book, Estimate Builder, AI Estimate, Repair Proposal, Customer Approval, Invoice, Deposit, or Payment work.
- This document is the primary product source of truth for Finance. Treat it as higher-level product guidance than individual task docs.
- Key language: technician-confirmed `Repair Solution`, customer-facing `Repair Proposal`, internal-only `Bundle`, and optional/internal `Itemized Estimate`.
- Customers must not work directly with Price Book or see Bundle internals.
- Complaint is not Diagnosis. AI must not diagnose, invent repair scope, invent prices, or override technician decisions.
- AI may format, organize, check completeness, and draft Proposal language from technician-confirmed scope and approved Price Book data.
- Keep Finance minimal. Do not build CRM, giant tables, long default lists, or permanent accounting controls inside the normal repair workflow.
- Task 167.9 changed documentation only. No production code, backend, UI, migrations, Price Book implementation, Finance implementation, Auth, or company membership data was changed.
- Task 168 has not started.

## Task 168 Repair Proposal Builder Design

- Read `docs/TASK168_REPAIR_PROPOSAL_BUILDER_PLAN.md` before implementing any Repair Proposal Builder work.
- Task 168 is design-only. It audited the current Job Workspace Finance tab, estimate APIs, public token approval page, invoice dependency, AI estimate agent, Price Book selection endpoint, and estimate/invoice migrations.
- Product naming is locked: technician uses `Repair Proposal Builder`; customer receives `Repair Proposal`; `Estimate` remains the internal financial entity.
- Preserve legacy contracts: existing `service_request_estimates`, `service_request_estimate_items`, public `/estimates/[token]` links, `send_service_request_estimate_to_customer_rpc`, `respond_to_public_estimate_rpc`, job status sync, and `create_invoice_from_estimate_rpc`.
- Future work should evolve existing estimate persistence and add only minimal proposal/version metadata needed for immutable customer snapshots.
- Do not build a second estimate engine, expose bundles to customers, let AI diagnose, or allow complaint-only Proposal generation.
- Task 168 changed documentation only. No production code, UI, backend, API routes, migrations, database schema, Auth, Price Book implementation, or Finance implementation was changed.
- Task 169 has not started.

## Task 168.1 Finance Data Contract Audit And Migration Plan

- Read `docs/TASK168_1_FINANCE_DATA_CONTRACT_AND_MIGRATION_PLAN.md` before writing any Finance data migration, Proposal Builder API, shared calculation engine, public Proposal page, or Price Book-to-Proposal integration.
- Task 168.1 is documentation-only. It intentionally did not change production code, UI, backend, API routes, RPCs, database schema, or migrations.
- Decision: use `service_request_estimates` as the V1 Repair Proposal version/internal Estimate row. Use `service_request_estimate_items` as the financial line snapshot plus future Repair Solution grouping table.
- Do not create `repair_proposal_versions` in V1 unless a later explicit architecture task reverses this decision.
- Future additive migration should add proposal snapshot columns, version/supersede columns, calculation snapshot columns, Repair Solution grouping columns, Price Book source reference/snapshot columns, checks, and indexes exactly from the plan unless the future implementer documents a concrete deviation.
- Keep token ownership on `service_request_estimates.public_approval_token_hash`; public approval must continue to resolve a specific estimate/proposal version row.
- Keep invoice conversion from approved `service_request_estimates`; do not create a second invoice path.
- Existing legacy estimates, customer approval links, job status sync, and invoice conversion are compatibility contracts.
- Task 168.2, Task 169, Shared Calculation Engine, Builder UI, Price Book integration into Proposal Builder, Proposal send, and customer Proposal page have not started.

## Task 168.2 Repair Proposal Builder First Production Vertical Slice

- Read `docs/TASK168_2_REPAIR_PROPOSAL_VERTICAL_SLICE.md` before continuing Finance UI, proposal, public approval, or invoice compatibility work.
- Main UI file: `frontend/src/components/dashboard/ServiceRequestDetail.tsx`.
- Public customer file: `frontend/src/components/public/PublicEstimateApproval.tsx`.
- The Finance tab now uses Repair Proposal Builder language, but the persistence path is still the existing estimate API/RPC. Do not create a parallel proposal engine.
- Technician workflow now supports confirmed repair scope entry, browser-native dictation when available, Generate Repair Proposal, editable Repair Solution/customer summary, editable line type/title/price/internal cost/quantity/taxability/description, line reordering, warranty editing, tax/discount adjustment, Save Draft, Preview, and Send.
- Send is blocked when confirmed scope, line titles, quantities, or customer prices are missing/invalid. Save Draft remains softer so incomplete drafts can be preserved.
- Public `/estimates/[token]` now renders as a Repair Proposal with itemized details behind `View Itemized Estimate`; legacy tokens still use the same route and approval endpoint.
- Internal cost, margin, Price Book IDs, AI information, technician notes, and bundle internals must stay out of the customer-facing main proposal page.
- No migration was created for Task 168.2. Task 168.1 remains the source for future additive schema work.
- Task 169, Payment, Stripe, Partial Approval, Multiple Proposal Alternatives, Full Price Book Management, Invoice Builder Redesign, Inventory, Vendor Ordering, Accounting Reports, Finance Settings, Dashboard Redesign, and Shared Calculation Engine have not started.

## Task 168.2B Repair Proposal QA Handoff

- Continue from `docs/TASK168_2_REPAIR_PROPOSAL_VERTICAL_SLICE.md` for exact verified QA details.
- QA account only was used for the final flow; do not use owner/non-QA accounts for future Finance QA.
- QA job: `33a16f94-0176-4378-8c56-1344ddee9dc3`.
- Approved proposal: `EST-2026-B13AF2AB`, total `$595.00`, three lines.
- Declined QA proposal: `EST-2026-D3DE3ADB`, total `$1.00`, used only to verify decline/status/timeline behavior.
- Invoice compatibility result: approved estimate `EST-2026-B13AF2AB` created draft invoice `INV-2026-05BD2E90`; duplicate invoice creation returned the same invoice and kept one invoice row.
- Code fix: `frontend/src/app/api/estimate-agent/draft/route.ts` now recognizes dispenser `supply tubing` / `water supply tubing` as an explicit water-line thawing scope when technician text includes thaw/defrost language.
- Code fix: `frontend/src/components/dashboard/ServiceRequestDetail.tsx` now shows a visible dictation error when browser speech recognition is unavailable or blocked.
- Known limitation: saved estimate totals still use existing estimate RPC totals. Builder preview tax/discount settings are not yet persisted as an authoritative calculation contract.
- Visual responsive screenshots were not captured from the Codex runtime after browser tooling reset; owner should confirm 375px, 390px, 430px, and desktop layouts in the existing local browser session.
- Task 168.3 and Task 169 have not started.

## Task 168.2C Authoritative Totals Handoff

- New migration to apply manually: `supabase/migrations/0067_repair_proposal_authoritative_totals_apply_ready.sql`.
- Codex could not apply 0067 from this environment because there is no `psql`, no Supabase CLI, and no safe SQL execution RPC.
- 0067 adds calculation snapshot fields to `service_request_estimates` and adds 4-argument create/update RPC overloads that persist discount, tax rate, taxable amount, non-taxable amount, tax, total, internal cost, gross profit, and margin.
- Shared calculation helper: `frontend/src/server/finance/repair-proposal-calculations.ts`.
- Focused tests: `frontend/src/server/finance/repair-proposal-calculations.test.ts`.
- API route `frontend/src/app/api/service-requests/[id]/estimates/route.ts` now sends `adjustments` to the new RPC overloads.
- Job Workspace `ServiceRequestDetail.tsx` now uses the shared helper for preview calculations and restores persisted discount/tax inputs when editing a saved draft.
- Public customer component `PublicEstimateApproval.tsx` now supports persisted discount/tax fields in the Itemized Estimate disclosure.
- Local checks passed: `node --test src/server/finance/repair-proposal-calculations.test.ts`, `npm run lint`, `npm run build -- --webpack`, and `git diff --check`.
- Remaining required QA after applying 0067: create QA Proposal with discount/tax, save, reload, preview, send, public Proposal, Itemized Estimate, approve, invoice, decline separate Proposal, and responsive checks at 375px, 390px, 430px, and desktop.
- Do not mark Task 168.2 complete and do not start Task 168.3 or Task 169 until final browser QA passes after 0067 is applied.

## Task 168.2D Final QA After 0067 Handoff

- Owner manually applied `0067_repair_proposal_authoritative_totals_apply_ready.sql`.
- Functional browser QA used only QA account `qa-booking-tech@example.test`; do not use owner/non-QA accounts for follow-up Finance QA.
- QA job: `33a16f94-0176-4378-8c56-1344ddee9dc3`.
- Authoritative proposal: `EST-2026-490987A9`.
- Approved invoice: `INV-2026-1EC31CFD`.
- Entered/verified values: subtotal `$595.00`, flat discount `$10.00`, tax rate `8.25%`, tax `$23.12`, total `$608.12`, internal cost `$200.00`, gross profit/margin displayed as `$408.12 · 67.11%`.
- Verified same `$608.12` through Builder Preview, saved card, reload/edit, Customer Preview, public Proposal, public Itemized Estimate, approved state, and invoice card.
- Public Itemized Estimate showed only customer-visible line items, subtotal, discount, tax, tax rate, total, and did not show internal cost, margin, markup, Price Book IDs, AI confidence, vendor cost, internal warnings, or technician notes.
- Decline regression passed with separate proposal `EST-2026-7DE52D2A`; public decline updated immediately, job status became `Waiting Customer`, and timeline captured decline/status events.
- Timeline captured create/send/status/approval/invoice events for `EST-2026-490987A9`.
- Legacy rows remained readable in the same job: `EST-2026-B13AF2AB`, `EST-2026-D3DE3ADB`, and `INV-2026-05BD2E90`. No pre-Task-168 raw historical approval token was available, so do not claim historical-token QA.
- Responsive visual QA is complete after macOS Screen Recording permission was granted. Codex verified true `375px`, `390px`, `430px`, and desktop `1280px` widths for Technician Repair Proposal Builder, sent public Repair Proposal, sent Itemized Estimate, approved public state, and approved Itemized Estimate. QA-only visual proposal `EST-2026-E3837895` was sent to verify reachable `Approve Proposal` / `Decline` controls before response. The approved proposal `EST-2026-490987A9` verified `$608.12` approved state plus itemized Subtotal `$595.00`, Discount `-$10.00`, Tax `$23.12`, Tax rate `8.25%`, and Total `$608.12`. No horizontal overflow, unreadable totals, wrapping failures, customer internal-data leaks, or discount/tax layout breaks were found.
- Task 168.2D is complete. Do not start Task 168.3 or Task 169 unless explicitly requested.

## Task 168.3 Repair Proposal Generation Contract Handoff

- Read `docs/TASK168_3_REPAIR_PROPOSAL_GENERATION_CONTRACT.md` before changing Repair Proposal generation, Estimate Agent, Price Book-to-Proposal behavior, manual Proposal creation, template Proposal creation, or future AI Proposal work.
- New server files:
...
- New API route: `frontend/src/app/api/repair-proposals/generate/route.ts`.
- Legacy adapter: `frontend/src/app/api/estimate-agent/draft/route.ts` now tries the new Repair Proposal generation contract first and still returns `draft` for the current Job Workspace Finance UI.
- Provider order is OpenAI -> Anthropic -> deterministic fallback. Use server-side `OPENAI_API_KEY` and `ANTHROPIC_API_KEY` only. Never expose, print, document, rename, or copy secret values.
- The deterministic fallback and scope enforcement preserve explicit technician work in English, Russian, Ukrainian, and mixed technician language. The Russian evaporator sealed-system phrase now produces explicit evaporator, filter drier, service valve, refrigerant recharge, evaporator labor, and testing labor lines.
- Focused tests: `cd frontend && node --loader ./test/node-ts-loader.mjs --test src/server/finance/repair-proposal-calculations.test.ts src/server/finance/repair-proposal-generation.test.ts`.
- No migration, UI redesign, Invoice Builder, Payments, Deposits, Dashboard, Timeline, `.env.local`, or Task 169 work was added.

## Task 168.4 Repair Proposal Builder UX Handoff

- Read `docs/TASK168_4_REPAIR_PROPOSAL_BUILDER_UX.md` before changing Job Workspace -> Finance proposal UX.
- Main UI file changed: `frontend/src/components/dashboard/ServiceRequestDetail.tsx`.
- The visible Finance builder now starts with Generate with AI, Create Manually, and Use Template. If AI scope is missing, a focused scope sheet collects technician-confirmed repair scope before calling the existing generation flow.
- Builder review is now section/card based: What we found, Repair Solution, What we will do, Included Items, Warranty, Estimated Completion, Totals, and Warnings.
- Custom proposal lines still use the existing `customEstimateLines` state and estimate API payload; each visible row opens a bottom sheet for edits.
- Sticky bottom actions are Preview, Save Draft, and Send. They still call the existing preview/save/send handlers.
- Do not change the Repair Proposal generation contract, AI prompt, customer Proposal page, approval flow, invoice conversion, or schema as part of follow-up UX polish unless explicitly requested.
- Task 169 has not started.

## Task 168.4A Repair Proposal Builder QA Handoff

- Final QA account: `qa-booking-tech@example.test` only. Do not use the owner account for follow-up Finance QA unless the owner explicitly asks.
- QA job: `33a16f94-0176-4378-8c56-1344ddee9dc3`.
- New QA draft saved during final QA: `EST-2026-BB50EAE4`, 7 lines, total `$1,921.83`.
- Active Builder fixes are in `frontend/src/components/dashboard/ServiceRequestDetail.tsx`: readable Included Item labels, customer visibility toggle, add/edit/delete checklist behavior, active Preview modal, and warning-aware Send validation.
- Preview modal should show only customer-facing proposal content and itemized customer prices. It must not show internal cost, margin, Price Book IDs, AI metadata, vendor cost, internal warnings, or customer visibility flags.
- Save Draft remains allowed for incomplete drafts. Send is disabled while warnings/blockers are present.
- Responsive browser checks passed at 375px, 390px, 430px, and desktop. No horizontal overflow was found.
- Known limitation: `Estimated Completion` is recorded in decision context on save, but there is not yet a dedicated persisted visible estimate field for reload/edit display.
- Task 168.5 and Task 169 have not started.

## Task 168.5 Manual Estimate Production Review Handoff

- Current owner-confirmed state: migrations `0068` through `0072` have been applied in Supabase, old Estimates are visible, new Manual Estimates save, technician manual approval works, and the current local UI is ready for production review.
- Address and Property Details fixes are part of this release. Job service address remains the source of truth for Property Details, Zillow/HasData, Maps, and Distance; customer address synchronization is secondary and must not block Job Workspace rendering.
- Property Details accepted root cause/fix: ZIP validation previously picked street number `20406`; the HasData adapter now uses the last ZIP-like token and supports the current HasData response shape/cache namespace.
- Manual Estimate editor lives in `frontend/src/components/dashboard/ManualEstimateEditor.tsx`. It is intentionally compact and Workiz-dense: customer/address/date context only, Items first, Totals second, What we found / Repair solution below, Warranty/Estimated completion/Notes/Attachments collapsed, and a single sticky bottom action bar.
- Do not re-add duplicate Job number inside the estimate context block. The Job number belongs in the Job Workspace header only.
- Manual approval endpoint: `POST /api/estimates/[id]/approve-for-customer`. It requires dashboard auth and calls `approve_service_request_estimate_for_customer_rpc`.
- Manual approval must remain distinct from customer approval. Use `approval_source = technician_manual` and technician-on-behalf timeline language. Do not route technician approval through the public token customer response RPC.
- Sending remains independent from approval. Approved estimates can still be sent/resend for customer records; send must not downgrade an approved estimate back to sent.
- Autosave and Sync to Job are intentionally not implemented in this release.

...
--
-- APPLY-READY SCOPE:
-- - Intended for a disposable development or staging Supabase project after 0001 is verified.
-- - Creates onboarding foundation tables, enums, constraints, indexes, triggers, and baseline RLS enablement.
-- - Does not create frontend behavior, server actions, invite acceptance logic, or final onboarding UI.
-- - Company creation, invite acceptance, join approval, verification, and onboarding completion still require future audited server-side mutations.
--
...
-- This migration assumes public.profiles, public.app_role, public.current_app_role(),
-- and public.set_updated_at() exist from 0001_profiles_roles.sql.
-- Confirm 0001 exists before running this migration.
...
comment on column public.profiles.onboarding_status is
  'Task 60/61 apply-ready staging migration addition. Tracks the next onboarding state; frontend must not treat this as authorization.';
...
comment on function public.prevent_unsafe_onboarding_profile_updates() is
  'Task 69 apply-ready staging guard. Review trigger ownership and admin bypass behavior before applying.';
...
comment on table public.companies is
  'Task 60/61 apply-ready staging migration. Company/team account for owner-scoped CRM, onboarding, dispatch, and billing later.';
...
comment on column public.companies.status is
  'Company lifecycle status. active/suspended/rejected require reviewer metadata in this staging migration.';
...
comment on table public.company_members is
  'Task 60/61 apply-ready staging migration. Authoritative company/team membership scope for owners, managers, dispatchers, and technicians.';
...
comment on table public.technician_profiles is
  'Task 60/61 apply-ready staging migration. Private technician onboarding, verification, matching, and marketplace eligibility record.';
...
comment on table public.company_invites is
  'Task 60/61 apply-ready staging migration. Company invitation records. Store only token hashes; raw invite tokens must never be persisted.';
...
comment on table public.company_join_requests is
  'Task 60/61 apply-ready staging migration. Technician-to-company join request workflow for owner review.';
...
-- Table-specific policies are created by 0010. If this migration is applied alone, authenticated
-- users still need reviewed grants/policies before these tables are usable.

-- RLS posture:
-- - RLS is enabled here; table-specific policies are supplied by 0010.
-- - Company creation, owner membership creation, invite token validation, join approval, technician verification, and onboarding completion remain server-side workflows.
-- - Raw invite tokens are never stored; only token_hash is persisted.
-- - Public technician pages must use sanitized projections rather than raw technician_profiles rows.

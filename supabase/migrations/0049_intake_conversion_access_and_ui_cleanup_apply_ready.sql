-- Task 150.2 - Intake conversion access and UI cleanup support
--
-- Forward-only compatibility patch after 0047/0048.
-- Goals:
--   1. Persist Apt / Unit / Suite as structured intake data.
--   2. Preserve first/last-name intake editing while keeping customer_name
--      generated for legacy service_request compatibility.
--   3. Harden conversion so a converted job is not inserted unless the current
--      dashboard user can access it through company scope or technician slug
--      scope.

alter table public.intake_requests
  add column if not exists unit text;

comment on column public.intake_requests.unit is
  'Task 150.2 structured Apt / Unit / Suite value captured during intake review.';
...
begin
  ...
  if nullif(p_payload->>'company_id', '') is not null then
    ...
    if not public.user_can_access_company(payload_company_id) then
...
comment on function public.convert_intake_request_rpc(uuid, boolean) is
  'Task 150.2 conversion hardening. Resolves company/technician access before inserting a job and only marks intake converted after the created job is dashboard-visible.';

-- Task XXX follow-up: finalize customer address normalized duplicate detection after live QA.
--
-- Scope:
-- - Fix state normalization before writes (Texas -> TX, not TE).
-- - Normalize ZIP to five digits before customer_addresses writes.
-- - Preserve original human-readable customer address when a duplicate variant is found.
-- - Keep Job service address as the source of truth; customer address sync is secondary.
--
-- Safety:
-- - Forward-only.
-- - Replaces RPC definitions only.
...
declare
  ...
  state_value text := nullif(public.normalize_customer_address_match_value(p_payload->>'state', 'state'), '');
...
comment on function public.upsert_customer_address_rpc(uuid, uuid, jsonb) is
  'Task XXX/0069 customer address write repair: dedupes normalized address variants, keeps original human-readable address when a duplicate is found, and normalizes state/ZIP before writes.';
...
comment on function public.convert_intake_request_rpc(uuid, boolean) is
  'Task XXX/0069 conversion repair: keeps Job service address as source of truth, syncs missing customer addresses only, and avoids overwriting existing human-readable customer address variants.';

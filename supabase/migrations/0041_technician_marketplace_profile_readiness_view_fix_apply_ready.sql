-- Task 147.1 continuation: Technician marketplace profile readiness view fix.
--
-- Forward-only apply-ready migration for dev/staging.
--
-- Why this exists:
--   Migration 0040 attempted to CREATE OR REPLACE public.public_technician_profiles
--   with new columns inserted before existing columns. PostgreSQL does not allow
--   CREATE OR REPLACE VIEW to change existing view column names/order, which
--   produced:
--     ERROR 42P16 cannot change name of view column "specialties" to "service_zip_codes"
--
-- Fix:
--   - Keep existing public_technician_profiles compatibility by preserving the
--     original column order.
--   - Append the new marketplace readiness columns after the original columns.
--   - Drop/recreate only the sanitized public view, not the underlying table.
--   - Re-apply the safe self-owned profile RPC shape in case 0040 failed before
...
select
  ...
  -- Original view columns stay in their original order for compatibility.
  tp.specialties,
  tp.languages,
  tp.years_experience,
  ...
  tp.created_at,
  -- Task 147.1 columns are appended.
  tp.service_cities,
...
comment on view public.public_technician_profiles is
  'Sanitized public technician profile projection for customer-facing pages and booking. Original columns remain compatible; Task 147.1 marketplace readiness fields are appended. Does not expose profile_id, company_id, private bio, verification internals, suspension/rejection/archive details, emails, phones, or audit data.';
